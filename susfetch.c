    ...
    if (argc > 1) {
        for (int i = 1; i < argc; i++) {
            if (strncmp(argv[i], "--color", 7) == 0 && argc > i + 1) {
                for (int j = 0; j < 10; j++) {
                    if (strncmp(argv[i + 1], colorss[j][0],
                                strlen(colorss[j][0])) == 0) {
...
#define LEN 300
    ...
    char *location = malloc(sizeof(*location) * (LOGIN_NAME_MAX * 2));
    sprintf(location, "%s%s", getenv("HOME"), "/.sussusfetch/AMOGUS.mp3");
